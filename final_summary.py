...
_ANSWER_RE   = re.compile(r'정답|해설')


def _file_category(fn: str) -> str:
    """파일 분류: 'target' | 'combine' | 'answer'"""
    if _ANSWER_RE.search(fn):
        return 'answer'
    if _COMBINE_RE.search(fn):
        return 'combine'
    return 'target'
...
def main():
    ...
    # ── 파일 분류 ─────────────────────────────────────────────────────────────
    # file_data: {fname: (ts, counts, details, bogi, image, stem, miss, total, category)}
    file_data = {}
    for fname, (ts, fr) in sorted(reports.items()):
        ...
        file_data[fname] = (ts, counts, details, bogi, image, stem, miss, total, cat)

    def _print_table(entries, *, include_grand=True):
        """entries: [(fname, ts, bogi, image, stem, miss, total)]"""
        col = f"{'파일':<50} {'보기':>4} {'이미지':>5} {'발문':>5} {'누락':>5} {'합계':>5} {'판정':>8}"
        print(f"\n{col}")
        print("-" * 82)
        grand = defaultdict(int)
        bucket_cnt = defaultdict(int)
        for fname, ts, bogi, image, stem, miss, total in entries:
            grand['bogi']  += bogi
            grand['image'] += image
            grand['stem']  += stem
            grand['miss']  += miss
        ...
        return grand, bucket_cnt

    target_entries  = [(f, ts, b, im, st, ms, tot)
                       for f, (ts,_,_,b,im,st,ms,tot,cat) in file_data.items() if cat == 'target']
    combine_entries = [(f, ts, b, im, st, ms, tot)
                       for f, (ts,_,_,b,im,st,ms,tot,cat) in file_data.items() if cat == 'combine']
    answer_entries  = [(f, ts, b, im, st, ms, tot)
                       for f, (ts,_,_,b,im,st,ms,tot,cat) in file_data.items() if cat == 'answer']

    # ── 섹션 1: 측정 대상 ─────────────────────────────────────────────────────
    print(f"\n{'='*80}")
    print(f"=== 측정 대상 ({len(target_entries)}개 파일) ===")
    print(f"{'='*80}")
    grand, bucket_cnt = _print_table(target_entries)

    # 통계
    print(f"\n{'='*40}")
    print("파일 판정 분포 (측정 대상)")
    print(f"{'='*40}")
    for label in ['✅ 완벽', '🟢 1-2건', '🟡 3-8건', '🔴 9건+']:
        cnt = bucket_cnt.get(label, 0)
        print(f"  {label:10s} {cnt:3d}개  {'█' * cnt}")
    print(f"  총 {len(target_entries)}개 파일")

    # ── 섹션 2: 측정 외 (합본) ────────────────────────────────────────────────
    if combine_entries:
        print(f"\n{'='*80}")
        print(f"=== 측정 외 (합본) ({len(combine_entries)}개 파일) — QA 집계 제외 ===")
        print(f"{'='*80}")
        _print_table(combine_entries, include_grand=False)

    # ── 섹션 3: 측정 외 (정답해설) ───────────────────────────────────────────
    if answer_entries:
        print(f"\n{'='*80}")
        print(f"=== 측정 외 (정답해설) ({len(answer_entries)}개 파일) ===")
        print(f"{'='*80}")
        _print_table(answer_entries, include_grand=False)

    # ── 이슈 유형별 분류 상세 (측정 대상만) ─────────────────────────────────
    print(f"\n{'='*80}")
    print("이슈 유형별 전체 집계 (측정 대상)")
    print(f"{'='*80}")
    type_total = defaultdict(int)
    for fname, (ts, counts, details, *_) in file_data.items():
        if _file_category(fname) != 'target':
            continue
    ...
    # ── 파일별 상세 (측정 대상, 이슈 있는 파일만) ────────────────────────────
    print(f"\n{'='*80}")
    print("파일별 이슈 상세 (측정 대상, 0건 파일 제외)")
    print(f"{'='*80}")
    for fname, (ts, counts, details, bogi, image, stem, miss, total, cat) in sorted(file_data.items()):
        if cat != 'target' or total == 0:
            continue
        print(f"\n  [{fname[-55:]}]  합계={total}")
    ...
    if baseline_map:
        print(f"\n{'='*80}")
        print("Baseline 대비 변화 (오늘 아침 → 현재, 측정 대상)")
        print(f"{'='*80}")
        improved = worsened = unchanged = 0
        for fname, (ts, counts, details, bogi, image, stem, miss, total, cat) in sorted(file_data.items()):
            if cat != 'target' or fname not in baseline_map:
                continue
            bl = baseline_map[fname]
            bl_total = sum(bl.values())
            diff = total - bl_total
