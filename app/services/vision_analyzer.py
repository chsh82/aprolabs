...
- labeled_ranges: [A], [B] 등 세로 괄호({, |, [ 형태)로 표시된 범위. start_words는 범위 시작 텍스트 첫 4단어, end_words는 마지막 4단어
- underlined_phrases: 밑줄이 그어진 어구 (정확히, 있는 그대로). 없으면 []
- 이 페이지에 해당 요소가 없으면 빈 배열 []
...
def apply_structure_to_text(raw_text: str, structure: dict) -> str:
    ...
    # 2. [A]/[B] 범위 마커 적용 (기존 탐지 없는 경우만)
    for rng in structure.get("labeled_ranges", []):
        label = rng.get("label", "A")
        if f"[{label}:START]" in text:
            continue  # 이미 geometric 탐지로 적용됨
