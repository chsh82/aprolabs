...
_DATE_TOKEN = (
    ...
    # 1994년(수능 첫 해)은 "N차 수능"으로 표기된다(예: "1994학년도 1차 수능") - 다른
    # 연도의 "YYYY.수능"/"YYYY.N" 표기와 형태가 달라 별도 대안으로 추가한다. 이 패턴이
    # 없으면 이 토큰에서 날짜 스트리핑 루프가 멈추고, 뒤따르는 실제 정의 텍스트까지
    # definition 필드에 남는다(phase5/phase6 보고서, 동병상련 등 10건에서 재현됨).
    r"|\d{4}학년도\s*\d차\s*수능"
