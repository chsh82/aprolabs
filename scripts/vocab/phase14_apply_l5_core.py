#!/usr/bin/env python3
"""Phase14: 48건 NEW_PRIVATE_LOADABLE 항목(L5)을 vocabulary_quiz_research.db에
단일 트랜잭션으로 적재한다(vocabulary_contents 신규 행 + vocabulary_content_levels
신규 행, 같은 트랜잭션). phase13_apply_l4_core.py를 L5용으로 최소 수정한 스크립트.
...
BATCH_ID = "SCHEMA_READING_PHASE14_L5_CORE_48"
MERGE_SOURCE = "SCHEMA_READING_PHASE14_L5_CORE_48"
SOURCE_VERSION = "schema_reading_literacy_l5_manual_v1"  # 의도적으로 2.1.29(SOURCE_VERSION)와도,
    # phase13의 l4_manual_v1과도 다른 값 - quiz.py/multiformat.py의 문항 매칭 조건과 절대
    # 겹치지 않게 하는 추가 방어선. L4 배치와도 구분되는 고유 마커.
GENERATION_METHOD = "MANUAL_STRUCTURED_AUTHORING"  # 기존 사용 값 재사용(phase13과 동일) - 사람이 직접
    # literacy.db 정의를 읽고 학생용 정의·예문을 작성했음을 정확히 반영
...
LEVEL_STATUS = "REVIEW_BOUNDARY"  # PROVISIONAL_AUTO/REVIEW_BOUNDARY 중 기존 값만 사용(새 값
    # 발명 금지 지시 준수). REVIEW_BOUNDARY를 택한 근거: 이 48건은 자동 채점 알고리즘을 거치지
    # 않았고(PROVISIONAL_AUTO는 자동 채점 신뢰도가 높은 경우), 사람이 이번 세션에서 처음
    # 작성·대조했을 뿐 교과 전문가 검수나 별도 QA 배치를 거치지 않았다 - "추가 검토가
    # 필요하다"는 라우터의 REVIEW_BOUNDARY 의미론에 더 부합한다(phase13과 동일 판단).
VOCAB_LEVEL = 5
TARGET_GRADE_BAND = "고등 1학년"  # docs/literacy/07-학년경계정책-L5L6.md(2026-09-24,
    # 사용자 확정): "L5=고1, L6=고2~3". terms.level=5는 이 정책에 따라 고1로 매핑된다
    # (grade_level 컬럼은 이 소스 행들에서 전부 NULL이라 개별 학년 태그는 없음 - 리스크로 기록).
LEVEL_SOURCE = "MANUAL_LITERACY_L5_MATCH"
...
def main() -> int:
    ...
    content_rows = []
    for r in loadable:
        content_id = f"SR_L5CORE_{r['literacy_term_id']}"
    ...
    existing = set()
    cur.execute("SELECT content_id FROM vocabulary_contents WHERE content_id LIKE 'SR_L5CORE_%'")
    ...
    try:
        ...
        for cid, r in to_insert:
            ...
            level_reason = {
                ...
                "phase12_group": r["group"],
                "is_S_subject_concept": r["is_S"],
                "subject_category": r["subject_category"],
                "note_week": r["note_week"],
                "caution": r["caution"],
                "loaded_by": "schema_reading_phase14_l5_core48",
                "note": "REVIEW_BOUNDARY: 사람이 직접 literacy.db 정의 대조 후 작성, 자동 채점/전문가 검수 미실시. "
                        "grade_level 컬럼이 NULL이라 '고1' 근거는 docs/literacy/07-학년경계정책-L5L6.md 정책(L5=고1)에 의존.",
            }
            cur.execute(
