#!/usr/bin/env python3
"""Phase13: 49건 NEW_PRIVATE_LOADABLE 항목을 vocabulary_quiz_research.db에
...
BATCH_ID = "SCHEMA_READING_PHASE13_L4_CORE_50"
MERGE_SOURCE = "SCHEMA_READING_PHASE13_L4_CORE_50"
SOURCE_VERSION = "schema_reading_literacy_l4_manual_v1"  # 의도적으로 2.1.29(SOURCE_VERSION)와
    # 다른 값 - quiz.py/multiformat.py의 문항 매칭 조건과 우연히도 절대 겹치지 않게 하는 추가 방어선
GENERATION_METHOD = "MANUAL_STRUCTURED_AUTHORING"  # 기존 사용 값 재사용(4893/5723행) - 사람이 직접
    # literacy.db 정의를 읽고 학생용 정의·예문을 작성했음을 정확히 반영
QA_METHOD = "DETERMINISTIC_PLUS_HEURISTIC"  # 기존 사용 값 재사용 - 결정론적 대조(정의 일치, 레벨
    # 재조회) + 휴리스틱 검토(동형이의어 스캔, 예문 자연스러움 직접 읽고 판단)를 반영
...
LEVEL_STATUS = "REVIEW_BOUNDARY"  # PROVISIONAL_AUTO/REVIEW_BOUNDARY 중 기존 값만 사용(새 값
    # 발명 금지 지시 준수). REVIEW_BOUNDARY를 택한 근거: 이 49건은 자동 채점 알고리즘을 거치지
    # 않았고(PROVISIONAL_AUTO는 자동 채점 신뢰도가 높은 경우), 사람이 이번 세션에서 처음
    # 작성·대조했을 뿐 교과 전문가 검수(S 14건, phase12 리스크3)나 별도 QA 배치를 거치지
    # 않았다 - "추가 검토가 필요하다"는 라우터의 REVIEW_BOUNDARY 의미론에 더 부합한다.
TARGET_GRADE_BAND = "중등 3학년"
...
def main() -> int:
    ...
    content_rows = []
    for r in loadable:
        content_id = f"SR_L4CORE_{r['literacy_term_id']}"
    ...
    existing = set()
    cur.execute("SELECT content_id FROM vocabulary_contents WHERE content_id LIKE 'SR_L4CORE_%'")
    ...
    try:
        ...
        for cid, r in to_insert:
            ...
            level_reason = {
                ...
                "caution": r["caution"],
                "loaded_by": "schema_reading_phase13_l4_core50",
            ...
            cur.execute(
                """INSERT INTO vocabulary_content_levels
                   (content_id, vocab_level, target_grade_band, level_score, level_confidence,
                    ...
                    is_active)
                   VALUES (?, 4, ?, NULL, NULL, ?, 1, 'MANUAL_LITERACY_L4_MATCH', ?, ?, 1)""",
                (
                    cid, TARGET_GRADE_BAND, LEVEL_STATUS, LEVEL_VERSION,
