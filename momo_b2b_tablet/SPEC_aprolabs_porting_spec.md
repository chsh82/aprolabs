...
| `qa` | 좌 (이미지 슬롯 → 문항 → 답란) / 우 제시문 | `ratio`(제시문:이미지 비율, 렌더러가 칸 순서에 맞춰 뒤집어 적용), `excerpt{p, text[], continued?}`, `q`, `slot?` |
...
| `compare` | 비교 카드 (각 카드 단일/번호형) | `cards[{title, prompt, n?}]`, `img?` | `{id}#1` 또는 `{id}#1-1…` |
...
- **STEP 2**
  ...
  - 공간 배분: 답란이 최대 줄 수까지 먼저 늘고, 그래도 30mm 이상 남으면 **문항 위쪽**에 이미지 슬롯이 열린다. 답란을 줄여 이미지를 넣는 일은 없다.
...
#### 3.4.2 좌우 배치 (2026-09-26)

전 교재 공통 규칙: **왼쪽 = 이미지 자리 + 문항 + 답란, 오른쪽 = 제시문.**
`qa`만 실제로 좌우 2칸이 바뀐다(`ratio`를 뒤집어 비율 유지). `qaband`/
`qaref`는 제시문이 좌우 칸이 아니라 상단 띠(band)라 이 규칙을 적용할
대상이 없어 그대로 둔다(안쪽 2칸은 이미 이미지·문항이 왼쪽). `solo`는
제시문이 없고 이미 이미지가 왼쪽이라 바꿀 게 없다. 책등 띠(spine)는
페이지 전체 기준 왼쪽 고정이라 이 규칙과 무관하다.

### 3.5 이미지

1. **원본 교재 이미지 우선.** DB `document_image`에 그 페이지 이미지가 있으면 그대로 쓴다(표지·배경·삽화).
2. 없으면 생성. 지시문 규칙:
   - 같은 페이지 제시문에 **명시된 장면만**.
